...
void InteractiveView::mouseMoveEvent(QMouseEvent *event)
{
    if (m_bMouseTranslate){
        //如果是右移
        //qDebug()<<m_lastMousePos.x()-event->pos().x();

        QPointF mouseDelta = mapToScene(event->pos()) - mapToScene(m_lastMousePos);
        mouseDelta.setX(mouseDelta.x()*-1);
      //  qDebug()<<mouseDelta.x();
    ...
    //qDebug()<<"InteractiveView::mouseMoveEvent";
...
void InteractiveView::zoomIn()
{
    ...
    if(myItem!=NULL){
      ...
      //  myItem->scale(1 + m_zoomDelta,1 + m_zoomDelta);
...
void InteractiveView::zoom(float scaleFactor)
{
    ...
    if(myItem!=NULL){
       ...
       // QRectF rect=myItem->rect();
        //myItem->setRect(rect.x(),rect.y(),rect.width()/scaleFactor,rect.height()/scaleFactor);
    }
}
